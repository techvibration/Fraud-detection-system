...
dataset_path = "D:\\Fraud-detection-system\\data\\creditcard_2023.csv"
fds_dataset = pd.read_csv(dataset_path)

#making the different datasets
y = fds_dataset['Class']
X = fds_dataset.drop(['Class','id'],axis = 1)
train_X,test_X,train_y,test_y = train_test_split(X,y,test_size = 0.2,stratify = y,random_state = 42) 

test_X.to_csv('X_val.csv', index=False)
test_y.reset_index(drop=True).to_frame(name='Class').to_csv('y_val.csv', index=False)

#using scale_pos_weight to increase recall
scale_pos_weight = (train_y == 0).sum() / (train_y == 1).sum()
adjusted_scale_pos_weight = scale_pos_weight * 1.5
...
joblib.dump(fd_model,'model1.pkl')
